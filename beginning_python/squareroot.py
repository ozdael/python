...
class SquareRootCalculator:
    """This class spawns a separate thread to calculate a bunch of square roots, and checks in it once a second until it finishes."""
    ...
    def __init__(self, target):
        ...
        while len(self.results) < target:
            ...
            time.sleep(1)
            print("Calculated %s square root(s); the last one is sqrt(%d)=%f" % (target, len(self.results), self.results[−1]))
